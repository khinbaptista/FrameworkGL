...
namespace FrameworkGL
{
    class Mesh
    {
        ...
        public static Mesh CreateDotFloorXZ(Vector3 start, Vector2 dimensions, float spacing, Color color) {
            Mesh floor = new Mesh();

            if (dimensions.X <= 0 || dimensions.Y <= 0)
                throw new ArgumentException("Both dimensions must be greater than zero", "dimensions");

            for (float x = start.X; x <= dimensions.X; x += spacing)
                for (float z = start.Z; z <= dimensions.Y; z += spacing) {
                    floor.AddVertex(new Vector3(x, start.Y, z));
                    floor.AddColor(color);
                }

            floor.SetUp();
            floor.DrawAsPoints = true;
        ...
        public static Mesh FromFile(string path) {
            ...
            StreamReader obj_file = new StreamReader(path);
            ...
            while (!obj_file.EndOfStream) {
                ...
                // if line starts with anything else, just ignore it for now (g, v, s, mtllib, usemtl...)
            }
